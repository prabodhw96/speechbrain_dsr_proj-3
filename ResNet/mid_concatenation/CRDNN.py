...
class CRDNN(sb.nnet.containers.Sequential):
    ...
    residual_size : int
        no. of CNN Blocks in residual blocks.
    mean : bool
        If True mean is taken after CNN (residual) layer
    n_microphones : int
        no. of microphones
    Example
    -------
    ...
    def __init__(
        ...
        if mean:
            self.append(Mean_layer(input_shape), layer_name="mean")
    ...
    @staticmethod
    def total_mics():
        return torch.tensor(n_mics)


class Mean_layer(sb.nnet.containers.Sequential):
    def __init__(self, input_shape):
        super().__init__(input_shape=input_shape)
        self.n_mics = int(CRDNN.total_mics())

    def forward(self, x):
        return torch.stack(list(torch.chunk(x, self.n_mics, 0))).mean(dim=0)
...
class CNN_Block(sb.nnet.containers.Sequential):
    ...
    residual_size : int
        no. of hidden layers in residual block.
    ...
    def __init__(
        ...
        j = 1
        for i in range(residual_size-2):
            self.append(
                sb.nnet.CNN.Conv2d,
                out_channels=channels,
                kernel_size=kernel_size,
            ...
            self.append(
                ...
                layer_name=f"norm_{j}",
            )
            self.append(activation(), layer_name="act_{j}")
            j = j + 1
        self.append(
            sb.nnet.CNN.Conv2d,
            out_channels=channels,
        ...
        self.append(sb.nnet.normalization.LayerNorm, layer_name=f"norm_{j}")
        self.append(activation(), layer_name=f"act_{j}")
        self.append(
    ...
    def forward(self, x):
        ...
        for layer in self.values():
            x = layer(x)
        if ((len(identity.shape) == 3) and (len(x.shape) == 4)):
            identity = identity.unsqueeze(dim=-1)
        x = x + identity
        return x
