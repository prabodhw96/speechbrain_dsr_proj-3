...
class ASR_Brain(sb.Brain):
    def add_noise(self, wavs, wav_lens):
        wavs_noise = self.hparams.env_corrupt(wavs, wav_lens)
        wavs = torch.cat([wavs, wavs_noise], dim=0)
        wav_lens = torch.cat([wav_lens, wav_lens])
        return wavs, wav_lens

    def compute_pout(self, wavs, wav_lens):
    ...
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            if hasattr(self.hparams, "env_corrupt"):
                wavs1, wav_lens1 = self.add_noise(wavs1, wav_lens1)
                wavs2, wav_lens2 = self.add_noise(wavs2, wav_lens2)
                wavs3, wav_lens3 = self.add_noise(wavs3, wav_lens3)
                wavs4, wav_lens4 = self.add_noise(wavs4, wav_lens4)

        pout1 = torch.exp(self.compute_pout(wavs1, wav_lens1)).unsqueeze(3)
        pout2 = torch.exp(self.compute_pout(wavs2, wav_lens2)).unsqueeze(3)
        pout3 = torch.exp(self.compute_pout(wavs3, wav_lens3)).unsqueeze(3)
        pout4 = torch.exp(self.compute_pout(wavs4, wav_lens4)).unsqueeze(3)
        pout = torch.stack([pout1, pout2, pout3, pout4]).mean(dim=0).squeeze(3)
        pout = torch.log(pout)
