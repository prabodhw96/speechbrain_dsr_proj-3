...
for i in dir_files:
    os.makedirs("TIMIT_DAS/" + i)

for i in range(len(timit_df)):
    source = timit_df["location"][i][9:]
    destination = "TIMIT_DAS/" + "/".join(source.split("/")[1:])
    speech_ds = delay_and_sum(source, show_plots=False)
...
train_df["wav"] = train_df["wav"].apply(
    lambda x: x.replace("TIMIT", "TIMIT_DAS")
)
train_df.to_csv("TIMIT_prepared/train.csv", index=False)

dev_df = pd.read_csv("TIMIT_prepared/dev.csv")
dev_df["wav"] = dev_df["wav"].apply(lambda x: x.replace("TIMIT", "TIMIT_DAS"))
dev_df.to_csv("TIMIT_prepared/dev.csv", index=False)

test_df = pd.read_csv("TIMIT_prepared/test.csv")
test_df["wav"] = test_df["wav"].apply(lambda x: x.replace("TIMIT", "TIMIT_DAS"))
test_df.to_csv("TIMIT_prepared/test.csv", index=False)

shutil.copy("TIMIT_prepared/train.csv", "TIMIT_DAS")
shutil.copy("TIMIT_prepared/dev.csv", "TIMIT_DAS")
shutil.copy("TIMIT_prepared/test.csv", "TIMIT_DAS")

for root, file, dirs in os.walk("TIMIT"):  # /path/to/TIMIT
    for dir in dirs:
        source = os.path.join(root, dir)
        if ".wav" not in source:
            destination = source.replace("TIMIT", "TIMIT_DAS")
            shutil.copyfile(
                source, destination
            )  # copy from /path/to/TIMIT to /path/to/TIMIT_DAS
