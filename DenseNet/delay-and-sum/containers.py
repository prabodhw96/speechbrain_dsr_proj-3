...
class Sequential_dense(torch.nn.ModuleDict):
    ...
    *layers, **named_layers
    ...
    >>> model.append(Linear, n_neurons=300, layer_name="layer3")
    ...
    def forward(self, x):
        """Concatenates the output of previous layers in sequence, passing them as input to the current layer.
        ...
        for name, layer in self.named_children():
            if "conv" in name.split("_"):
                temp = []
                for i in range(1, int(name.split("_")[-1])):
                    temp.append(act_dict["act_{}".format(i)])
